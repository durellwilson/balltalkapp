  ...
  useEffect(() => {
    const loadVerificationRequests = async () => {
      ...
      try {
        ...
        // Ensure all items have id property as non-optional and include required properties
        const requestsWithIds = requests.map(req => ({
          ...req,
          id: req.id || 'unknown-id', // Fallback if id is undefined
          documents: req.documents || [], // Ensure this is included
          // Ensure leagueAffiliation and teamAffiliation are correctly referenced
          leagueAffiliation: req.league || 'Unknown League', // Adjust as necessary
          teamAffiliation: req.team || 'Unknown Team', // Adjust as necessary
        }));
        setPendingVerifications(requestsWithIds); // Set the state with the requests
  ...
  const handleApprove = async (verification: AthleteVerification & { id: string }) => {
    ...
    try {
      const success = await VerificationService.approveVerificationRequest(
        verification.id,
        user.uid,
        'verified' // Changed to a valid status
      );
